...
public class CustomStyledPlayerView extends StyledPlayerView implements GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener {
    ...
    public CustomStyledPlayerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        exoErrorMessage.getCompoundDrawables();
        exoProgress = findViewById(R.id.exo_progress);
        setTextDrawColor(exoErrorMessage, PlayerActivity.savedColor);

        mScaleDetector = new ScaleGestureDetector(context, this);

    }

    private void setTextDrawColor (TextView textView, int color) {
    ...
    public boolean tap() {
        ...
        if (!PlayerActivity.controllerVisibleFully) {
            showController();
            return true;
        } else if (PlayerActivity.haveMedia && PlayerActivity.playerVid != null && PlayerActivity.playerVid.isPlaying()) {
            hideController();
            return true;
        }
        return false;
    ...
    public boolean onScroll(MotionEvent motionEvent, MotionEvent motionEvent1, float distanceX, float distanceY) {
        if (mScaleDetector.isInProgress() || PlayerActivity.playerVid == null || PlayerActivity.locked)
            return false;
        ...
        if (gestureOrientation == Orientation.HORIZONTAL || gestureOrientation == Orientation.UNKNOWN) {
            ...
            if (Math.abs(gestureScrollX) > SCROLL_STEP || (gestureOrientation == Orientation.HORIZONTAL && Math.abs(gestureScrollX) > SCROLL_STEP_SEEK)) {
                ...
                if (gestureOrientation == Orientation.UNKNOWN) {
                    if (PlayerActivity.playerVid.isPlaying()) {
                        restorePlayState = true;
                        PlayerActivity.playerVid.pause();
                    }
                    clearIcon();
                    seekStart = PlayerActivity.playerVid.getCurrentPosition();
                    seekChange = 0L;
                    seekMax = PlayerActivity.playerVid.getDuration();
                }
                ...
                if (PlayerActivity.haveMedia) {
                    if (gestureScrollX > 0) {
                        if (seekStart + seekChange - SEEK_STEP  * distanceDiff >= 0) {
                            PlayerActivity.playerVid.setSeekParameters(SeekParameters.PREVIOUS_SYNC);
                            seekChange -= SEEK_STEP * distanceDiff;
                            position = seekStart + seekChange;
                            PlayerActivity.playerVid.seekTo(position);
                        }
                    } else {
    ...
    public void setIconVolume(boolean volumeActive) {
        exoErrorMessage.setCompoundDrawablesWithIntrinsicBounds(volumeActive ? R.drawable.volumexml : R.drawable.mutexml, 0, 0, 0);
    }
    ...
    public void setIconBrightness() {
        exoErrorMessage.setCompoundDrawablesWithIntrinsicBounds(R.drawable.brightnessxml, 0, 0, 0);
    }

    public void setIconBrightnessAuto() {
        exoErrorMessage.setCompoundDrawablesWithIntrinsicBounds(R.drawable.brightness_autoxml, 0, 0, 0);
    }

    public void setIconLock(boolean locked) {
        exoErrorMessage.setCompoundDrawablesWithIntrinsicBounds(locked ? R.drawable.lockxml : R.drawable.unlockxml, 0, 0, 0);
    }
